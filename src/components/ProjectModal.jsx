...
export default function ProjectModal({ project, onClose }) {
  ...
  // Read translations live so personality toggle updates content
    ...
    <>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6 animate-fade-in">
        ...
        >
          ...
          <button
            onClick={onClose}
            className="sticky top-4 float-right mr-4 z-10 w-9 h-9 rounded-[10px] border bg-black/50 backdrop-blur-[10px] text-base flex items-center justify-center cursor-pointer hover:bg-black/70 transition"
            style={{ borderColor: "rgba(255,255,255,0.15)", color: "#94a3b8" }}
          >×</button>

          {/* Cover - flush to top, no rounding on top edges (parent handles it) */}
          <div
              ...
              <div className="flex flex-wrap gap-2.5 mb-6">
                {project.links.map((l, i) => (
                  <a key={i} href={l.href} target="_blank" rel="noopener noreferrer"
                    className="modal-link font-body px-4 py-2.5 rounded-[10px] border text-[13px] font-semibold no-underline transition-all flex items-center gap-1.5"
                    style={{
                      borderColor: personality ? "rgba(245,158,11,0.2)" : "var(--border)",
